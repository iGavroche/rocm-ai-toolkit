...
def is_model_quantized(model: torch.nn.Module) -> bool:
    """Check if a model is already quantized by looking for QLinear, QConv2d, etc."""
...
def quantize_model(
    ...
    # NOTE: This is a basic check - models loaded from disk are never quantized,
    # so this will only catch cases where quantization was done in a previous step
    # in the same session. For persistent caching, we'd need to save/load quantized
    # models to disk, which is not currently implemented.
    if is_model_quantized(model_to_quantize):
        base_model.print_and_status_update("Model appears to already be quantized, skipping quantization")
        # Still patch state_dict for saving
